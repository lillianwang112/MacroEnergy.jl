struct Steelmaking <: AbstractAsset
    id::AssetId
    steelmaking_transform::Transformation
    dri_edge::Edge{DRI}
...
function default_data(t::Type{Steelmaking}, id=missing, style="full")
    if style == "full"
...
function full_default_data(::Type{Steelmaking}, id=missing)
...
function simple_default_data(::Type{Steelmaking}, id=missing)
    return OrderedDict{Symbol,Any}(
        :id => id,
        :location => missing,
        :can_expand => true,
...
function make(asset_type::Type{Steelmaking}, data::AbstractDict{Symbol,Any}, system::System)
    ...
    steelmaking_key = :transforms
    @process_data(
        transform_data,
        data[steelmaking_key],
        [
            (data[steelmaking_key], key),
            (data[steelmaking_key], Symbol("transform_", key)),
            (data, Symbol("transform_", key)),
            (data, key),
        ]
    )
    steelmaking_transform = Transformation(;
        id = Symbol(id, "_", steelmaking_key),
    ...
    @start_vertex(
        ...
        dri_edge_data,
        DRI,
        [(dri_edge_data, :start_vertex), (data, :location)],
    )
    dri_end_node = steelmaking_transform
    dri_edge = Edge(
    ...
    ironore_end_node = steelmaking_transform
    ironore_edge = Edge(
    ...
    coal_end_node = steelmaking_transform
    coal_edge = Edge(
    ...
    natgas_end_node = steelmaking_transform
    natgas_edge = Edge(
    ...
    hydrogen_end_node = steelmaking_transform
    hydrogen_edge = Edge(
    ...
    steelscrap_end_node = steelmaking_transform
    ...
    elec_end_node = steelmaking_transform
    elec_edge = Edge(
    ...
    co2_start_node = steelmaking_transform
    @end_vertex(
        co2_end_node,
        co2_edge_data,
        CO2,
    ...
    co2_captured_start_node = steelmaking_transform
    @end_vertex(
    ...
    crudesteel_start_node = steelmaking_transform
    @end_vertex(
        crudesteel_end_node,
        crudesteel_edge_data,
        CrudeSteel,
    ...
    # stoichiometry
    steelmaking_transform.balance_data = Dict(
        :dri_consumption => Dict(
            crudesteel_edge.id => get(transform_data, :dri_consumption, 0.0),
            dri_edge.id => 1.0
        ),
        ...
        :coal_consumption => Dict(
            crudesteel_edge.id => get(transform_data, :coal_consumption, 0.0),
            coal_edge.id => 1.0
        ),
        ...
        :hydrogen_consumption => Dict(
            crudesteel_edge.id => get(transform_data, :hydrogen_consumption, 0.0),
        ...
        :emissions => Dict(
            crudesteel_edge.id => get(transform_data, :emission_rate, 0.0) * (1 - get(transform_data, :capture_rate, 0.0)),
            co2_edge.id => -1.0,
        ),
        :capture => Dict(
            crudesteel_edge.id => get(transform_data, :emission_rate, 0.0) * get(transform_data, :capture_rate, 0.0),
            co2_captured_edge.id => -1.0,
        )
    )

    return Steelmaking(id,
        steelmaking_transform,
        dri_edge,
        ironore_edge,
        coal_edge,
        natgas_edge,
